...
namespace NPilotHUD
{
    ...
    public partial class NHudControl : UserControl
    {
        ...
        private void DrawGroundSpeed(double AS, double GS, int command)
        {
            ...
            cache.GroundSpeed.tb.Text = GS.ToString("##0");
            if (IsInRange(int.Parse(cache.GroundSpeed.tb.Text), Round(command) - GroundSpeedCommandTolerance, Round(command) + GroundSpeedCommandTolerance))
            {
